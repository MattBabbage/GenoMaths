...
x = symbols('x')
symbol_list = [x, x]
fittest, score, best_array, average_array, q1_array, q3_array, fittest_array, timings = genetic_algorithm(symbol_list, fitness, 200, 40, 0.8, 0.2, df)
pd.DataFrame([best_array, average_array, q1_array, q3_array, timings]).to_csv("output.csv")

types = [Mul, Add]
ex_array = []
#Add Goal
ex_array.append(-15 * x ** 2 + x ** 3 + 20)
#Add Attempts
